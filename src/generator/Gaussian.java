...
public class Gaussian {
    ...
    public static void main(String[] args) {
        ...
        double seqLen = 100;
        double freq = 10;
        double interval = (mu+sigma)-(mu-sigma);
        double ei = (interval)/freq;
        System.out.println("ei = "+ei);
        List<Double> eiValues = new ArrayList<>();
        for (int i = 0; i < freq; i++) {
            z = mu-sigma + i*ei;
            double estimation = cdf(z, mu, sigma);
            System.out.println("\n\nz="+z+" estimation ="+estimation);
            double y = cdf(z);
            System.out.println(inverseCDF(y));
            System.out.println("value = "+String.valueOf((int) (estimation*seqLen)));
        }
